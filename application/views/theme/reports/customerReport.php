...
<form method="post" class="form-group" action="<?= reportUrl("cr") ?>">
...
<script>
    $(document).ready(function () {
        ...
        $(window).resize(function () {
            window.customerChart().redraw();
        });
    });

    function customerChart() {
        window.customerChart = Morris.Bar({
            element: 'customerChart',
            data:  <?= json_encode($customerChart) ?>,
            xkey: 'label',
            xLabelAngle: 65,
            ykeys: ['profit', 'item', 'sale'],
            labels: ["Revenue", "Items", "Sale",],
            barColors: ['#881596', '#B715C2', "#0373fc"],
            lineWidth: '3px',
            parseTime: false,
            grid: false,
            resize: true,
